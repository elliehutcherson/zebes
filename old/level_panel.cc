...
absl::StatusOr<std::unique_ptr<LevelPanel>> LevelPanel::Create(Options options) {
  ...
  if (options.editting_level == nullptr) {
    return absl::InvalidArgumentError("Editting level must not be null.");
  }
  return absl::WrapUnique(new LevelPanel(std::move(options)));
}

LevelPanel::LevelPanel(Options options)
    : api_(*options.api), editting_level_(*options.editting_level) {
  RefreshLevelCache();
}
...
absl::StatusOr<LevelResult> LevelPanel::RenderList() {
  ...
  if (ImGui::Button("Create")) {
    ...
    *editting_level_ = {
    ...
    ASSIGN_OR_RETURN(result, HandleOp(Op::kLevelCreate));
  }
  ImGui::SameLine();
  ...
  if (ImGui::Button("Delete")) {
    ASSIGN_OR_RETURN(result, HandleOp(Op::kLevelDelete));
    Detach();
  }
  ImGui::PopStyleColor();
...
absl::StatusOr<LevelResult> LevelPanel::RenderDetails() {
  ...
  if (ImGui::Button("Save", ImVec2(button_width, 0))) {
    ASSIGN_OR_RETURN(result, HandleOp(Op::kLevelUpdate));
  }
  ImGui::SameLine();
  ...
  if (ImGui::Button("Reset", ImVec2(button_width, 0))) {
    ASSIGN_OR_RETURN(result, HandleOp(Op::kLevelReset));
  }
  ImGui::PopStyleColor();
...
absl::StatusOr<LevelResult> LevelPanel::HandleOp(Op op) {
  if (selected_index_ >= static_cast<int>(level_cache_.size())) {
    selected_index_ = -1;
    return absl::OutOfRangeError("Selected index is out of range");
  }

  if (!editting_level_.has_value() && op != Op::kLevelDelete) {
    return absl::InternalError("No level being edited");
  }

  LevelResult result;
  auto refresh = absl::MakeCleanup([&] { RefreshLevelCache(); });

  if (op == Op::kLevelDelete) {
    // Do nothing if no level is selected.
    if (selected_index_ == -1) return result;

    std::string id = level_cache_[selected_index_].id;
    RETURN_IF_ERROR(api_.DeleteLevel(id));
    Detach();  // Clear selection
    return LevelResult{.type = LevelResult::kDetach, .level_id = id};
  }

  if (op == Op::kLevelUpdate) {
    if (editting_level_->name.empty()) {
      return absl::InvalidArgumentError("Level name cannot be empty");
    }
    if (editting_level_->width <= 0 || editting_level_->height <= 0) {
      return absl::InvalidArgumentError("Level dimensions must be greater than 0");
    }
  ...
  if (op == Op::kLevelCreate) {
    ...
    *editting_level_ = {
    ...
    return LevelResult{.type = LevelResult::Type::kAttach, .level_id = id};
  }
  ...
  return absl::InvalidArgumentError("Unknown op!!!!");
}
