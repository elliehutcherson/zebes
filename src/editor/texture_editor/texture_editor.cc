...
void TextureEditor::Render() {
  ...
  // Drawn every frame, outside the table: the dialog outlives the frame that
  // opened it, and only reports a path on the frame the user confirms.
...
void TextureEditor::RenderTextureDetails() {
  ...
  if (model_.is_new_texture() && gui_->Button("Create")) {
    ...
    if (!loaded.ok()) {
      ...
      model_.SetError(
          absl::StrCat("Created the texture but could not reload it: ", loaded.status().message()));
    } else {
