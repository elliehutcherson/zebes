...
TileMatrix::TileMatrix(const GameConfig *config)
    : config_(config), x_size_(config_->tiles.size_x),
      y_size_(config_->tiles.size_x),
...
int TileMatrix::Get(int x, int y) const { return matrix_[x + (x_size_ * y)]; }
