...
Vector Distance(const Point& p1, const Point& p2) {
  return Vector(p1 - p2);
};
...
float Polygon::x_min() const {
  return vertices_[x_min_index_].x;
}

int Polygon::x_min_floor() const {
  return vertices_[x_min_index_].x;
}

float Polygon::x_max() const {
  return vertices_[x_max_index_].x;
}

int Polygon::x_max_floor() const {
  return vertices_[x_max_index_].x;
}

float Polygon::y_min() const {
  return vertices_[y_min_index_].y;
}

int Polygon::y_min_floor() const {
  return vertices_[y_min_index_].y;
}

float Polygon::y_max() const {
  return vertices_[y_max_index_].y;
}

int Polygon::y_max_floor() const {
  return vertices_[y_max_index_].y;
}
...
const absl::flat_hash_map<uint8_t, AxisDirection>* Polygon::GetPrimaryAxes() const {
  return &primary_axes_;
}

std::optional<AxisOverlap> Polygon::GetOverlapOnAxis(
  const Polygon& other, const Vector& axis) const {
...
absl::Status Polygon::AddPrimaryAxisIndex(uint8_t index, AxisDirection axis_direction) {
  if (index >= vertices_.size()) 
    return absl::OutOfRangeError("Index out of range when tryting to add primary axis.");
