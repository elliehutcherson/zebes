...
SDL_Rect* Sprite::GetSource() {
  return &sources_[index_];
}

const SDL_Rect* Sprite::GetSource(int index) const {
  return &sources_[index];
}

SpriteType Sprite::GetType() const {
  return sprite_config_.type;
}

const SpriteConfig* Sprite::GetConfig() const {
  return &sprite_config_;
}
...
uint64_t Sprite::GetCycle() const {
  return cycle_;
}
