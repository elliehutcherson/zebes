...
absl::StatusOr<std::unique_ptr<SpriteManager>> SpriteManager::Create(const Options& options) {
  if (options.config == nullptr) return absl::InvalidArgumentError("GameConfig is null.");
  if (options.renderer == nullptr) return absl::InvalidArgumentError("SDL_Renderer is null.");
  if (options.camera == nullptr) return absl::InvalidArgumentError("Camera is null.");
  if (options.db == nullptr) return absl::InvalidArgumentError("DbInterface is null.");
...
absl::Status SpriteManager::Init() {
  ASSIGN_OR_RETURN(std::vector<std::string> texture_paths, db_->GetAllTexturePaths());
  for (const std::string& texture_path : texture_paths) {
    RETURN_IF_ERROR(InitializeTexture(texture_path));
  }

  ASSIGN_OR_RETURN(std::vector<SpriteConfig> sprite_configs, db_->GetAllSprites());

  for (SpriteConfig& sprite_config : sprite_configs) {
    ASSIGN_OR_RETURN(std::vector<SpriteFrame> sprite_frames,
                     db_->GetSpriteFrames(sprite_config.id));
    sprite_config.sprite_frames = std::move(sprite_frames);
...
absl::Status SpriteManager::InitializeTexture(const std::string& texture_path) {
  LOG(INFO) << __func__ << ": Initializing texture, texture_path: " << texture_path;
  auto texture_iter = path_to_texture_.find(texture_path);
  if (texture_iter != path_to_texture_.end()) {
    LOG(WARNING) << __func__ << ": Texture already initialized, texture_path: " << texture_path;
    return absl::OkStatus();
  }

  SDL_Surface* tmp_surface = IMG_Load(texture_path.c_str());
  if (tmp_surface == nullptr) return absl::AbortedError("Failed to create tmp_surface.");

  SDL_Texture* texture = SDL_CreateTextureFromSurface(renderer_, tmp_surface);
  if (texture == nullptr) return absl::AbortedError("Failed to create texture.");
...
absl::Status SpriteManager::InitializeSprite(SpriteConfig sprite_config) {
  LOG(INFO) << __func__ << ": Initializing sprite, sprite_id: " << sprite_config.id
            << ", sprite_type: " << SpriteTypeToString(static_cast<SpriteType>(sprite_config.type));
  if (sprite_config.size() <= 0) {
    return absl::InternalError(
        absl::StrCat(__func__, "Sprite Config has invalid size, sprite_id: ", sprite_config.id));
  }

  auto texture_iter = path_to_texture_.find(sprite_config.texture_path);
  if (texture_iter == path_to_texture_.end()) {
    return absl::NotFoundError(
        absl::StrCat("Texture not found, path: ", sprite_config.texture_path));
  }
...
absl::Status SpriteManager::DeleteSprite(uint16_t sprite_id) {
  ...
  if (it == sprites_.end()) {
    return absl::NotFoundError(absl::StrCat("Sprite not found, sprite_id: ", sprite_id));
  }
...
const absl::flat_hash_map<std::string, SDL_Texture*>* SpriteManager::GetAllTextures() const {
...
absl::StatusOr<const SpriteInterface*> SpriteManager::GetSprite(uint16_t sprite_id) const {
...
absl::StatusOr<const SpriteInterface*> SpriteManager::GetSpriteByType(uint16_t sprite_type) const {
  auto it = type_to_sprites_.find(sprite_type);
  if (it == type_to_sprites_.end()) {
    return absl::NotFoundError(absl::StrCat("Sprite not found, sprite_type: ", sprite_type));
  }
  return it->second;
}
...
absl::Status SpriteManager::Render(uint16_t sprite_id, int index, Point position) {
  auto it = sprites_.find(sprite_id);
  if (it == sprites_.end()) return absl::InvalidArgumentError("Type not found.");

  SpriteInterface* sprite = it->second.get();
  const SpriteConfig* config = sprite->GetConfig();

  if (index >= config->size()) return absl::InvalidArgumentError("Index out of bounds.");

  const SpriteFrame* sprite_frame = sprite->GetSpriteFrame(index);

  const SDL_Rect dst_rect{
      .x = position.x_floor() + sprite_frame->render_offset_x,
      .y = position.y_floor() + sprite_frame->render_offset_y,
      .w = sprite_frame->render_w,
      .h = sprite_frame->render_h,
  };
