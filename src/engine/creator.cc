...
void Creator::Render() {
  ...
  for (const auto& [_, polygon] : tiles_) {
    camera_->RenderLines(*polygon->vertices(), DrawColor::kColorTile, /*static_position=*/false);
  }
}
...
void Creator::ToggleTileState() {
  ...
  tiles_[mouse_world_tile_position_] = std::make_unique<Polygon>(GetTileVertices());
}
