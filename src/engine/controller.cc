...
absl::StatusOr<std::unique_ptr<Controller>> Controller::Create(
    Controller::Options options) {
  if (options.save_config == nullptr) {
    return absl::InvalidArgumentError("Save config must not be null.");
  }
  return std::unique_ptr<Controller>(new Controller(std::move(options)));
}

Controller::Controller(Options options)
    : save_config_(std::move(options.save_config)) {}

void Controller::HandleInternalEvents() {
  for (InternalEvent &event : internal_events_) {
    switch (event.type) {
      case InternalEventType::kCreatorSaveConfig:
        save_config_(std::get<GameConfig>(event.value));
        break;
