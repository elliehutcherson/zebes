...
struct AxisOverlap {
  ...
  double min_magnitude() const { return left_magitude() < right_magitude() ? left_magitude() : right_magitude(); }
  double max_magnitude() const { return left_magitude() > right_magitude() ? left_magitude() : right_magitude(); }
  double primary_magitude() const { 
    if (primary_axis_direction == AxisDirection::axis_left) return left_magitude();
    if (primary_axis_direction == AxisDirection::axis_right) return right_magitude();
    return min_magnitude();
  }
 
  double min_distance() const { return left_magitude() < right_magitude() ? left_distance : right_distance; }
  double max_distance() const { return left_magitude() > right_magitude() ? left_distance : right_distance; }
  double primary_distance() const { 
    if (primary_axis_direction == AxisDirection::axis_left) return left_distance;
    if (primary_axis_direction == AxisDirection::axis_right) return right_distance;
    return min_distance();
  }
  ...
  double primary_overlap_x() const { 
    if (primary_axis_direction == AxisDirection::axis_left) return left_overlap_x();
    if (primary_axis_direction == AxisDirection::axis_right) return right_overlap_x();
    return min_overlap_x();
  }
  
  double primary_overlap_y() const { 
    if (primary_axis_direction == AxisDirection::axis_left) return left_overlap_y();
    if (primary_axis_direction == AxisDirection::axis_right) return right_overlap_y();
  ...
  std::string ToString() const {
    ...
    absl::StrAppend(&message,  absl::StrFormat("min_magnitude: %f\n", min_magnitude()));
    absl::StrAppend(&message, absl::StrFormat("min_overlap_x: %f\n", min_overlap_x()));
    absl::StrAppend(&message, absl::StrFormat("min_overlap_y: %f\n", min_overlap_y()));
    absl::StrAppend(&message,  absl::StrFormat("max_magnitude: %f\n", max_magnitude()));
    absl::StrAppend(&message, absl::StrFormat("max_overlap_x: %f\n", max_overlap_x()));
    absl::StrAppend(&message, absl::StrFormat("max_overlap_y: %f\n", max_overlap_y()));
    return message;
  }
};
...
struct PolygonOverlap {
  ...
  int min_overlap_magnitude() const {
    return abs(min_overlap_distance()); 
  }
  ...
  std::string ToString() const {
    ...
    absl::StrAppend(&message, absl::StrFormat("min_overlap_y: %f\n", min_overlap_y()));
    if (has_primary) {
      const AxisOverlap& primary_axis_overlap = axis_overlaps[min_primary_overlap_index];
      absl::StrAppend(&message, absl::StrFormat("min_primary_overlap_distance: %f\n", min_primary_overlap_distance()));
      ...
      absl::StrAppend(&message, absl::StrFormat("min_primary_axis_direction: %d\n", primary_axis_overlap.primary_axis_direction));
    }
    int axis_index = 0;
    for (const AxisOverlap& axis_overlap : axis_overlaps) {
      absl::StrAppend(&message, absl::StrFormat("axis_index: %d\n", axis_index++));
...
class Polygon {
 public:
  ...
  std::optional<AxisOverlap> GetOverlapOnAxis(const Polygon& other, const Vector& axis) const;
  ...
  absl::Status AddPrimaryAxisIndex(uint8_t index, AxisDirection axis_direction = AxisDirection::axis_none);
  // Move all vertices. 
