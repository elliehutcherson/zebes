...
struct CollisionArea {
  ...
  std::string DebugString() {
    return absl::StrFormat("id: %d, x_min: %f, x_max: %f, "
    "y_min: %f, y_max: %f", id, x_min, x_max, y_min, y_max);
  }
};
...
class CollisionManager {
 ...
 private:
  ...
  absl::flat_hash_map<uint64_t, absl::flat_hash_set<int>> object_id_to_area_ids_;
  absl::flat_hash_map<int, CollisionArea> areas_;
  absl::flat_hash_map<int, absl::flat_hash_set<uint64_t>> area_id_to_object_ids_;
