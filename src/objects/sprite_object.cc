...
absl::StatusOr<std::unique_ptr<SpriteObject>> SpriteObject::Create(ObjectOptions& options) {
  if (options.vertices.empty()) return absl::InvalidArgumentError("Vertices must not be empty.");
...
absl::Status SpriteObject::AddSprite(const SpriteInterface* sprite) {
  ...
  if (sprites_.contains(sprite_id)) return absl::AlreadyExistsError("Profile already exists.");
...
absl::Status SpriteObject::RemoveSprite(uint16_t sprite_id) {
  ...
  if (profile_iter == sprites_.end()) return absl::NotFoundError("Profile not found.");
...
absl::Status SpriteObject::SetActiveSprite(uint16_t sprite_id) {
  if (!sprites_.contains(sprite_id)) return absl::NotFoundError("Profile not found.");

  active_sprite_id_ = sprite_id;
  active_sprite_ticks_ = 0;
  active_sprite_frame_index_ = 0;
  return absl::OkStatus();
}
...
uint8_t SpriteObject::GetActiveSpriteTicks() const { return active_sprite_ticks_; }

uint64_t SpriteObject::GetActiveSpriteCycles() const { return active_sprite_cycles_; }

uint16_t SpriteObject::GetActiveSpriteIndex() const {
  if (active_sprite_id_ == 0) return 0;
  return active_sprite_frame_index_;
}
...
void SpriteObject::Update() {
  ...
  const SpriteFrame* frame = sprite->GetSpriteFrame(active_sprite_frame_index_);

  // Increment ticks for the current frame.
  active_sprite_ticks_++;
  if (frame->frames_per_cycle == 0) return;

  // If the current frame has been displayed for the correct number of ticks,
  // move to the next frame.
  if (active_sprite_ticks_ >= frame->frames_per_cycle) {
    active_sprite_ticks_ = 0;
    active_sprite_frame_index_++;

    // If we have reached the end of the animation, loop back to the start.
    if (active_sprite_frame_index_ >= sprite->GetConfig()->size()) {
      active_sprite_frame_index_ = 0;
      active_sprite_cycles_++;
    }
  }
};
