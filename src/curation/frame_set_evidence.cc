...
absl::StatusOr<RgbaImage> RenderEnlargedFrame(const SpriteFrame& frame, const RgbaImage& texture) {
  ...
  ASSIGN_OR_RETURN(
      RgbaImage enlarged,
      CreateCheckerboardRgbaImage(kCanvas, kCanvas, 16, kReviewCheckerLight, kReviewCheckerDark));
...
absl::StatusOr<RgbaImage> RenderAnimationStrip(const Sprite& sprite, const RgbaImage& texture) {
  ...
  ASSIGN_OR_RETURN(RgbaImage strip,
                   CreateCheckerboardRgbaImage(strip_width, strip_height, 8, kReviewCheckerLight,
                                               kReviewCheckerDark));
  int x = kGutter;
...
absl::StatusOr<RgbaImage> RenderContactSheet(const Sprite& sprite, const RgbaImage& texture) {
  ...
  ASSIGN_OR_RETURN(RgbaImage sheet,
                   CreateCheckerboardRgbaImage(columns * kCell, rows * kCell, 8,
...
absl::StatusOr<DifferenceArtifact> RenderDifference(const SpriteFrame& from, const SpriteFrame& to,
  ...
  RETURN_IF_ERROR(ValidateSpriteFrameGeometry(from, texture));
  RETURN_IF_ERROR(ValidateSpriteFrameGeometry(to, texture));
...
absl::StatusOr<CurationReview> BuildFrameSetReview(const FrameSetReviewIdentity& identity,
  ...
  ASSIGN_OR_RETURN(RgbaImage strip, RenderAnimationStrip(sprite, texture));
  ASSIGN_OR_RETURN(RgbaImage contact_sheet, RenderContactSheet(sprite, texture));
  ASSIGN_OR_RETURN(RgbaImage alignment_sheet, RenderAlignmentSheet(sprite, texture));
  ...
  for (const SpriteFrame& frame : sprite.frames) {
    ASSIGN_OR_RETURN(RgbaImage native, CropFrame(frame, texture));
    ASSIGN_OR_RETURN(RgbaImage enlarged, RenderEnlargedFrame(frame, texture));
  ...
  auto add_difference = [&](size_t from_index, size_t to_index, bool loop_closure) -> absl::Status {
    ASSIGN_OR_RETURN(DifferenceArtifact difference,
                     RenderDifference(sprite.frames[from_index], sprite.frames[to_index], texture));
    const std::string id =
  ...
  if (sprite.playback_mode == SpritePlaybackMode::kLoop) {
    ...
    ASSIGN_OR_RETURN(RgbaImage hold_final, RenderEnlargedFrame(sprite.frames.back(), texture));
    review.artifacts.push_back({
        .id = "hold-final",
