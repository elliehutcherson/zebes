...
public:
    Bitmap(int width, int height)
        : width_(width), height_(height), data_(height, std::vector<uint8_t>(width * 3, 255)) {}
    ...
    bool saveToBMP(const std::string &filename) const {
        ...
        fileHeader.bfSize = sizeof(BitmapFileHeader) + sizeof(BitmapInfoHeader) + width_ * height_ * 3;
        ...
        for (int y = height_ - 1; y >= 0; --y) {
            outFile.write(reinterpret_cast<const char *>(data_[y].data()), width_ * 3);
        }
