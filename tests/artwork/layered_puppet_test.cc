...
// The arm is drawn 4 long from the shoulder and the pose halves that bone to 2.
// Rigid, the artwork keeps its length and hangs past the posed hand.
TEST(LayeredPuppetTest, ARigidBoneKeepsItsArtworkLengthWhenThePoseShortensIt) {
  absl::StatusOr<LayeredPuppet> puppet = TestPuppet();
  ASSERT_OK(puppet);
  puppet->poses[1].joints = {{.x = 5, .y = 5}, {.x = 5, .y = 9}, {.x = 7, .y = 5}};

  const absl::StatusOr<RgbaImage> rendered = RenderLayeredPuppetPose(*puppet, puppet->poses[1]);
  ASSERT_OK(rendered);
  EXPECT_EQ(rendered->pixels[Offset(9, 5) + 3], 255) << "rigid artwork should still reach x 9";
}

TEST(LayeredPuppetTest, AStretchingBoneScalesItsArtworkToThePosedLength) {
  absl::StatusOr<LayeredPuppet> puppet = TestPuppet();
  ASSERT_OK(puppet);
  puppet->bones[1].may_stretch = true;
  puppet->poses[1].joints = {{.x = 5, .y = 5}, {.x = 5, .y = 9}, {.x = 7, .y = 5}};

  const absl::StatusOr<RgbaImage> rendered = RenderLayeredPuppetPose(*puppet, puppet->poses[1]);
  ASSERT_OK(rendered);
  EXPECT_EQ(rendered->pixels[Offset(9, 5) + 3], 0) << "halved arm should not reach x 9";
  EXPECT_EQ(rendered->pixels[Offset(6, 5) + 3], 255) << "halved arm should still cover x 6";
}

TEST(LayeredPuppetTest, StretchLeavesAPoseThatKeepsItsBoneLengthAlone) {
  const absl::StatusOr<LayeredPuppet> rigid = TestPuppet();
  absl::StatusOr<LayeredPuppet> stretching = TestPuppet();
  ASSERT_OK(rigid);
  ASSERT_OK(stretching);
  stretching->bones[0].may_stretch = true;
  stretching->bones[1].may_stretch = true;

  // The contact pose turns the arm without changing its length, so allowing a
  // stretch has nothing to scale and both renders must be identical.
  const absl::StatusOr<RgbaImage> one = RenderLayeredPuppetPose(*rigid, rigid->poses[1]);
  const absl::StatusOr<RgbaImage> two = RenderLayeredPuppetPose(*stretching, stretching->poses[1]);
  ASSERT_OK(one);
  ASSERT_OK(two);
  EXPECT_EQ(one->pixels, two->pixels);
}
...
TEST(LayeredPuppetTest, MaskedArtworkAndSubtractionArePixelComplements) {
  ...
  ASSERT_OK(SubtractLayeredPuppetMask(left, *mask));
...
TEST(LayeredPuppetTest, StretchNeverOverwritesPaintedPixels) {
  ...
  ASSERT_OK(StretchLayeredPuppetBackfill(layer, required, 8, 0));
...
TEST(LayeredPuppetTest, StretchDrawsBodyColourNotContourColour) {
  ...
  ASSERT_OK(StretchLayeredPuppetBackfill(from_contour, required, 8, 0));
  RgbaImage from_interior = layer;
  ASSERT_OK(StretchLayeredPuppetBackfill(from_interior, required, 8, 1));
...
TEST(LayeredPuppetTest, DownsampleAndPackKeepFrameContract) {
  ...
  ASSERT_OK(neutral_frame);
  ASSERT_OK(contact_frame);
