...
Point WidthVelocity(const Registration& registration, Point point) {
  ...
  for (Point pin : registration.target) {
    nearest_pin_squared = std::min(nearest_pin_squared, (point.x - pin.x) * (point.x - pin.x) +
  ...
  for (const WidthControl& width : registration.widths) {
    ...
    const double weight =
        std::pow(std::cos(along * 1.5707963267948966), 2) * std::exp(-std::pow(transverse, 4));
...
void Write(std::ostream& stream, const Registration& registration) {
  ...
  if (!registration.widths.empty()) {
    ...
    for (size_t i = 0; i < registration.widths.size(); ++i) {
      ...
      stream << "{\"center\":[" << width.center.x << ',' << width.center.y << "],\"normal\":["
             << width.normal.x << ',' << width.normal.y
             << "],\"reference_half_width\":" << width.reference_half_width
             << ",\"scale\":" << width.scale << ",\"half_length\":" << width.half_length << '}';
    }
    stream << ']';
  }
  stream << ",\"mesh\":[";
  ...
  for (int y = 0; y <= 512; y += 4) {
    for (int x = 0; x <= 512; x += 4) {
      grid.push_back(Sample(registration, {static_cast<double>(x), static_cast<double>(y)}));
    }
  }
  bool first = true;
