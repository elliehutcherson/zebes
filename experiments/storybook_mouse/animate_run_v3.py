...
def close_hands(obj, rig):
    ...
    for vertex in obj.data.vertices:
        ...
        curl = Vector((x*(1-.30*close),
                       -.13+.105*math.tanh((y+.13)/.16),
                       -.015+.10*math.tanh((z+.015)/.10)))
...
def pose(obj, rig, geometry, phase):
    ...
    for side, offset, sign in SIDES:
        ...
        angle = arm['swing']
        upper = Vector((sign*.70, -math.sin(angle), -math.cos(angle))).normalized()
        lower = Vector((sign*.35, -math.sin(angle+arm['bend']), -math.cos(angle+arm['bend']))).normalized()
        ...
        rest_hand = rig.data.bones[f'hand.{side}']
        hand_rotation = (rest_hand.tail_local-rest_hand.head_local).rotation_difference(hand)
        # Palms face inward, so the run presents compact knuckles rather than
        # an upward-facing palm. The source model has no individual digit rig.
        rotated_bone(rig, f'hand.{side}', rig.pose.bones[f'forearm.{side}'].tail.copy(),
                     Quaternion(hand, sign*1.05) @ hand_rotation)
...
def diagnostics(obj, rig, geometry):
    ...
    hock_vertices = {i for i,w in enumerate(weights) if .24 < obj.data.vertices[i].co.z < .84 and
                     sum(value for name,value in w.items() if name.startswith(('shin.','foot.'))) > .02}
    return {'edges': edges, 'hock_vertices': hock_vertices,
...
def measure(scene, obj, rig, geometry, phase, diag):
    ...
    hock_ratios = sorted(ratio for ratio,a,b in edges if a in diag['hock_vertices'] or b in diag['hock_vertices'])
    ...
    record = {'phase': phase, 'frame': 1+phase*24, 'lowest_z': min(p.z for p in coords),
              ...
              'hock_edge_ratio': {'min': hock_ratios[0], 'p99': hock_ratios[len(hock_ratios)*99//100],
                                   'max': hock_ratios[-1]},
