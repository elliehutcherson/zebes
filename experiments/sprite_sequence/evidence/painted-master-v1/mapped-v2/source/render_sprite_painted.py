...
def render(binary, inputs, output, landmark_path=LANDMARKS, geometry_path=None):
    if output.exists():
        raise ValueError("output exists; choose a new revision to preserve evidence")
    geometry_path = geometry_path or inputs / "geometry.json"
    ...
    for index, (frame, posed) in enumerate(zip(document["frames"], geometry["frames"], strict=True)):
        ...
        run = subprocess.run([str(binary.resolve())], input=text, text=True, capture_output=True)
        if run.returncode:
    ...
    write_json(output / "manifest.json", {
        ...
        "sampling": "C++ inverse thin-plate mesh from explicit semantic pairs; two pins use similarity; full RGB/alpha artwork sampled once at 512px with bicubic interpolation. No guide clipping or procedural paint.",
...
if __name__ == "__main__":
    ...
    render(args.registration, args.inputs, args.output, args.landmarks, args.geometry)
