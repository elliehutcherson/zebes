...
def main():
    ...
    for index, frame in enumerate(masters):
        ...
        metadata.append({'index': index, 'blender_frame': index+1, 'phase': index/24,
                         'time_seconds': index/36, 'bounds_512': box,
    ...
    for size in (96, 128, 512):
        ...
        if size < 512:
            for step in (1, 2):
                subset = frames[::step]
                durations = [round((i+1)*1000*report['cycle_seconds']/len(subset))
                             - round(i*1000*report['cycle_seconds']/len(subset)) for i in range(len(subset))]
                subset[0].save(out/f'run-{size}-{len(subset)}.apng', save_all=True,
                               append_images=subset[1:], duration=durations, loop=0, disposal=0, blend=0)
    for size in (128, 384):
        ...
        durations = [10*(round((i+1)*100*report['cycle_seconds']/24)-round(i*100*report['cycle_seconds']/24))
                     for i in range(24)]
    ...
    review = {'status': 'awaiting_user_run_verdict', 'frames': metadata,
              'cycle_seconds': report['cycle_seconds'], 'full_fps': 36, 'sparse_fps': 18,
