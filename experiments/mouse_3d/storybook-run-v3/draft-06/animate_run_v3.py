...
def close_hands(obj, rig):
    ...
    for vertex in obj.data.vertices:
        ...
        if distance <= 0:
            continue
        theta = min(2.8, distance/.085)
        curl = Vector((x*(1-.20*smooth(.05,.22,distance)),
                       -.09-.085*math.sin(theta),
                       z+.085*(1-math.cos(theta))))
    ...
    return {'name': key.name, 'value': 1, 'changed_vertices': changes,
            'method': 'Continuous distal-hand curl; Basis, topology, UVs and texture preserved'}
