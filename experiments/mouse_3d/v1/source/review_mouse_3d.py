...
</style><main><div class="eyebrow">Zebes / Character experiment / 01</div><h1>One mouse. Every frame.</h1>
<p>A rigged 3D interpretation of the green-coated mouse, rendered into ordinary transparent sprites. This is an initial modeling and animation study; the painted finish and final character proportions remain open for review.</p>
<div class="grid"><section class="panel"><header>Appearance reference<span>Original artwork</span></header><div class="stage"><img class="art" src="source.png" alt="Original green hooded mouse"></div></section>
...
function draw(){paint(render,512);paint(pixel,Number(document.getElementById('resolution').value));slider.value=f;document.getElementById('counter').textContent=`${String(f+1).padStart(2,'0')} / 24`;
...
play.onclick=()=>{playing=!playing;play.textContent=playing?'Pause':'Play'};slider.oninput=()=>{f=Number(slider.value);playing=false;play.textContent='Play';draw()};document.getElementById('resolution').onchange=draw;document.getElementById('rig').onchange=draw;requestAnimationFrame(tick);
</script></main></html>"""
