...
from motion import FPS, FRAMES, HIP_HALF_WIDTH, SHOULDER_HALF_WIDTH, pose_at, rest_bones
from forms import EYE_X, EYE_Z, HEAD_SECTIONS, fur_color, head_point, head_side, iris_color, shorten_muzzle, trouser_radius
...
def build_trousers(side, hip, knee, ankle):
    rows, columns = 100, 64
    vertices, faces, paint = [], [], []
    knee_t = (hip.z-knee.z) / (hip.z-ankle.z)
    for row in range(rows+1):
        t = 1-row/rows
        center = hip.lerp(knee, t/knee_t) if t <= knee_t else knee.lerp(ankle, (t-knee_t)/(1-knee_t))
        for column in range(columns):
            angle = column*math.tau/columns
            radius, fold = trouser_radius(t, angle, side)
            vertices.append((center.x+radius*math.cos(angle), center.y+radius*0.94*math.sin(angle), center.z))
            outer = -math.pi/2 if side == "near" else math.pi/2
            distance = math.atan2(math.sin(angle-outer), math.cos(angle-outer))
            seam = 0.11*math.exp(-(distance/0.045)**2)
            shade = 1 + fold*1.4 - seam
            paint.append((0.23*shade, 0.15*shade, 0.078*shade, 1))
    for row in range(rows):
        faces.extend((row*columns+j, row*columns+(j+1)%columns,
                      (row+1)*columns+(j+1)%columns, (row+1)*columns+j) for j in range(columns))
    faces.extend([tuple(reversed(range(columns))), tuple(rows*columns+j for j in range(columns))])
    obj = mesh(f"Trousers_{side}", vertices, faces, surface_material(f"Trouser_fabric_{side}", (0.23, 0.15, 0.078)),
               limb_weights(side, "thigh", "shin", knee.z, 0.28), 1)
...
def build_limbs(colors):
    for side in ("far", "near"):
        ...
        build_trousers(side, hip, knee, ankle)
...
def build_clothes(colors):
    ...
    for obj in PARTS[first_part:]:
        for vertex in obj.data.vertices:
            vertex.co.x = 0.05 + (vertex.co.x - 0.05) * BODY_DEPTH
            vertex.co.y *= BODY_WIDTH
...
def build_lowered_hood(colors):
    ...
    for obj in PARTS[first_part:]:
        for vertex in obj.data.vertices:
            vertex.co.x = 0.05 + (vertex.co.x - 0.05)*1.09
            vertex.co.y *= 1.10
...
def build_eye(side, sign, colors):
    ...
    count, rings = 64, 18
    vertices, faces, rim, paint = [], [], [], []
    for ring in range(rings+1):
        radius = ring / rings
        for i in range(count):
            angle = i * math.tau / count
            u, v = radius*math.cos(angle), radius*math.sin(angle)
            x = EYE_X + 0.117*u - 0.018*v
            z = EYE_Z + 0.132*v*(0.90+0.10*abs(math.sin(angle)))
            y = head_side(x, z, sign) + sign*(0.013 + 0.036*(1-radius**2))
            vertices.append((x, y, z))
            paint.append(iris_color(u, v, radius))
            if ring == rings:
    ...
    eye_material = surface_material(f"Warm_iris_{side}", (0.11, 0.05, 0.02))
    eye_shader = eye_material.node_tree.nodes.get("Principled BSDF")
    eye_shader.inputs["Roughness"].default_value = 0.30
    eye_shader.inputs["Specular IOR Level"].default_value = 0.22
    eye_material.node_tree.links.remove(eye_shader.inputs["Normal"].links[0])
    eye = mesh(f"Eye_lens_{side}", vertices, faces, eye_material, "head", 1)
    paint_surface(eye, paint)
    tube(f"Upper_eyelid_{side}", rim[:count//2+1], 0.010, colors["fur_dark"], "head")
    tube(f"Lower_eyelid_{side}", rim[count//2:]+rim[:1], 0.006, colors["fur"], "head")
    for index, (dx, dz, rx, rz) in enumerate(((-0.020, 0.047, 0.022, 0.028), (0.034, -0.034, 0.008, 0.011))):
        x, z = EYE_X+dx, EYE_Z+dz
        sphere(f"Eye_glint_{side}_{index}", (x, head_side(x, z, sign)+sign*0.048, z),
               (rx, 0.007, rz), colors["white"], "head")
...
def build_head(colors):
    ...
    for obj in PARTS[first_part:]:
        for vertex in obj.data.vertices:
            vertex.co.x = shorten_muzzle(vertex.co.x)
...
def verify_closed_surface(object_name):
    obj = bpy.data.objects[object_name]
    topology = bmesh.new()
    topology.from_mesh(obj.data)
    try:
        if any(not edge.is_manifold for edge in topology.edges):
            raise ValueError(f"Surface must be closed and manifold: {object_name}")
        ...
        while stack:
            ...
            for edge in vertex.link_edges:
                ...
                if other in remaining:
                    ...
                    stack.append(other)
        if remaining:
            raise ValueError(f"Surface has disconnected component geometry: {object_name}")
...
def main():
    ...
    build_head(colors)
    build_tail(colors)
    head_check = verify_closed_surface("Head_continuous")
    cloth_checks = {side: verify_closed_surface(f"Trousers_{side}") for side in ("near", "far")}
    records = animate(rig)
    ...
    if args.preview:
        ...
        render(scene, args.out / "pants-detail.png", 4, (6, -20, 3), 2.25, (0.05, 0, 1.10))
    else:
        for frame in range(1, FRAMES + 1):
    ...
    manifest = {"version": 1, "design_revision": "v3-face-and-fabric", "blender": bpy.app.version_string, "fps": FPS,
                ...
                "cloth_surface_checks": cloth_checks,
                "design": {"hood": "lowered onto upper back", "torso_depth_multiplier": BODY_DEPTH,
                           "torso_width_multiplier": BODY_WIDTH, "hip_half_width": HIP_HALF_WIDTH,
                           "shoulder_half_width": SHOULDER_HALF_WIDTH,
                           "nose_tip_x": shorten_muzzle(1.17), "previous_nose_tip_x": 1.17,
                           "eye": "rounder warm iris, large pupil, soft lids, paired highlights",
                           "trousers": "continuous skinned surfaces with localized geometric folds"},
