...
from study_material import bake_study_material
...
def load_mesh(path):
    ...
    ground_correction = -min(vertex.co.z for vertex in obj.data.vertices)
    for vertex in obj.data.vertices:
        vertex.co.z += ground_correction
    ...
    return obj, {'imported_faces': original_faces, 'study_faces': len(obj.data.polygons),
                 ...
                 'post_reduction_ground_translation': ground_correction,
                 'height': 4.3}
...
def main():
    ...
    parser.add_argument('--color-mode', choices=('baked','authored','projected'), default='baked')
    ...
    if args.color_mode == 'baked':
        color, projection = bake_study_material(obj, args.out)
    else:
    ...
    for source in (Path(__file__), Path(__file__).with_name('study_rig.py'), Path(__file__).with_name('study_material.py')):
